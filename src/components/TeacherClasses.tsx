...
import { getTerms, getClassesForTeacher, getSchedulesForTeacher } from '../services/api';
...
export default function TeacherClasses() {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      ...
      setError(null);
      try {
        const response = await getClassesForTeacher(selectedTerm);
        if (response.status) {
  ...
  const fetchSchedules = async () => {
    ...
    setError(null);
    try {
      const response = await getSchedulesForTeacher(selectedTerm);
      if (response.status) {
